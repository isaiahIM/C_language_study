...
void UpdateMap(Size MapSize, char map[][MapSize.Length]);// 맵 출력 
void MoveDot(Size BlockSize, char block[][BlockSize.Length], Pos Dot, Pos RemovePos);// 블럭 이동 
void gotoxy(int x, int y);// 좌표 이동 

char ReadKey(void);// 키 입력 

int ChkCollision(Size MapSize, Size BlockSize,  char map[][MapSize.Length], char block[][BlockSize.Length], Pos Dot);// 충돌 검사 (하단:1, 오른쪽:2, 왼쪽:3, 충돌x:0) 
...
int main(int argc, char *argv[]) 
{
	...
	UpdateMap(MapSize, map);// 맵 초기출력 
	
	while(1)
	{	
		CollisionState = ChkCollision(MapSize, BlockSize, map, block,  Dot);// 퉁돌 확인
		
		if( CollisionState == DOWNCOLLISION)// 하단 충돌이 발생하면 
		{
			Stack(MapSize, map, Dot);// 별을 쌓고 
			UpdateMap(MapSize, map);// 맵을 업데이트함 
			Clear(&toggle);	
			i=2;
...
int ChkCollision(Size MapSize, Size BlockSize,  char map[][MapSize.Length], char block[][BlockSize.Length], Pos Dot)
{
	...
	for(i=0; i<BlockSize.Height; i++)
	{
		for(j=0; j<BlockSize.Length; j++)
		{
			if(map[Dot.Y+2][Dot.X+j])// 하단 충돌 
			{
				return DOWNCOLLISION;
			}
			
			else if(map[Dot.Y+i][Dot.X+2])// 우측 충돌 
			{
				return RIGHTCOLLISION;
			}
			
			else if(map[Dot.Y+i][Dot.X-1])// 좌측 충돌 
			{
